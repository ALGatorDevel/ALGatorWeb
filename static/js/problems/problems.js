...
var sectionTitle = ['Problem Overview',   'Implementation',  'Algorithms', 'Results and Analysis', 'Results playground',  'Source code'     ]
var sections     = ['projectDescription', 'editProject',     'algorithms', 'results',              'playground',          'implementation'  ];
var sectionIcons = ['fa-diagram-project', 'fa-wrench',       'fa-gears',   'fa-chart-column',      'fas fa-cog fa-pulse', 'fa-pen-to-square']
...
function getSideBarItemHTML(i) {
 ...
 <div id='${sections[i]}-sbitem' class='row' onclick="showSectionSideNavbar('${sections[i]}')">
    <p class='sideBarEl wide'>
      <span><i class="fa-solid ${sectionIcons[i]}"></i></span>
      <span>${sectionTitle[i]}</span>
    </p>
    <p class='sideBarEl slim' data-tooltip="${sectionTitle[i]}" onmouseover="showTooltip(event)" onmouseout="hideTooltip()">
      <span><i class="fa-solid ${sectionIcons[i]}"></i></span>
    </p>
  </div>
  `;
}
function getSideBarItemsHTML(topItems) {
  var topD = "", bottomD="";
...
//  if (sectionId == "results") repaintViews();
//  if (sectionId == "editProject") setEditPageHeight();
}
...
function getProblemDescriptionTitleDiv(sectionID, title, content) {
  let okCancelButtons = getOkCancelButtonsHTML(sectionID, false, "position:relative; top:10px; right:-8px");
  return `
    ...
    <div id="${sectionID}" class="problemDescTab" style="display: block;">
      <div class='w3-row' id="title_${sectionID}">
        ...
        <div class='w3-col s4' style="float:inline-end;">                   
          <div class="editMode" style="margin-right:30px">
            ${okCancelButtons}
            <i id="editButton_${sectionID}" class="fa-solid fa-pen-to-square icon" onclick="editDescriptionHtml('${sectionID}')" 
               style="float:right; display: flex; position:relative; top:40px;"></i>
          </div>
        </div>
      </div>  
...
/*  callDjangoWithAjax(
    ...
    function(response) {
      if (response.Status == 0)
        editProblemDescriptionSavePhase2(sectionID, htmlContent, response.newHtml);    
    }, null
  );
*/  
}
...
function testsetLineHTML(testset) {
  return `
    <li>
    <div>
      <p>${testset.name}: ${testset.description}</p>
    </div>
    </li>
  `;
}
