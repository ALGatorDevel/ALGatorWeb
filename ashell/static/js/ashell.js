...
function callDjango(url, blockID, callback, question='', param1=null, param2=null) {  
  ...
  $.ajax({
    ...
    error: function(xhr, status, error) {
      alert("Ajax askASServer error: " + error)
...
class ProjectListData extends Data {
  ...
  postProcess() {
    ...
    askASServer("getProjectList", this.blockID, (blockID, answer)=>{
...
class FilesData extends Data {
  ...
  showFileInEditor(projectName, fileName) {
    ...
    if (fp != -1) {
      ...
      askASServer('getfile {"Project":"'+projectName+'", "File":"'+fileName+'"}', this.blockID, (blockID, content) => {
  ...
  postProcess() {
    ...
    askASServer('getPFIles {"Project":"'+this.projectName+'"}', this.blockID, jQuery.proxy(this.loadTree, this));
  }  
}
...
class ResultsData extends Data {
  ...
  setData() {
    ...
    this.shStatus   = new Map(); // ali je vrstica vrstica prikazana (display none ali block)?
  ...
  setSHstatus(line, status) {
    shStatus.set(line, status);
  }
  ...
  getRunButton(project, algorithm, testset, mtype, family, computer) {
    return `<img class="rFile" title="Add task" src="/static/images/run24.png" onclick="addTask(this);" project="${project}" algorithm="${algorithm}" testset="${testset}" family="${family}" mt="${mtype}" computer="${computer}" />`;    
  }


  getSHbutton(blockID, idx) {
    var disp = this.shStatus.get(idx);
    var png  = (disp && disp.indexOf('block') >= 0) ? "/static/images/minus24.png" : "/static/images/plus24.png";
    return `<span id="spanSH-${this.blockID}-${idx}" onClick="toggleSH('${this.blockID}','${idx}');" class="spanSH"><img src="${png}" width=12 /></span>`; 
  }
  ...
  displayStatus(index, element) {
    ...
    // get information about the default file and default task
    var resIcon = stsIcon[results.DFTS.RS]; 
        resIcon = `<img src="/static/images/${resIcon}24.png" width=12/>`;
    var fileProg = (results.DFTS.RS == 0) ? "" : `<span class="rFile" onclick="editFile(this);" project="${this.data.Project}" fName="results/${results.DFTS.Fmy}.${results.DFTS.Cmp}/${results.Algorithm}-${results.TestSet}.${results.MType}">${results.DFTS.FS}</span>`;
    var fileName = results.DFTS.Fmy && results.DFTS.Cmp ? `${results.DFTS.Fmy}.${results.DFTS.Cmp}` : "";

    var runTaskB = this.getRunButton(this.data.Project, results.Algorithm, results.TestSet, results.MType, "", "");
    var shButton = this.getSHbutton(this.blockID, idx);

    var displayArray = this.shStatus.get(idx) ? this.shStatus.get(idx) : 'none';
    var eCont =  `<td class="tdRes" style="width:150px; display:block;">${shButton}${results.Algorithm}</td><td class="tdRes">${results.TestSet}</td><td class="tdRes">${results.MType}</td><td class="tdResLast">`;
        eCont += `<table class="resTab"><tr><td style="width:120px; text-align:left;">${results.DFTS.DF}</td>`;
        eCont += `<td class="resTD" width=150px;>${resIcon} ${fileName} ${fileProg}</td>`;
        eCont += `<td class="resTD" width=350px;>&nbsp;${results.DFTS.TS}</td><td>${runTaskB}</td></tr></table>`;        
        eCont += `<table class="resTab" style="display:${displayArray};" id="famTable-${this.blockID}-${idx}">`;
    for(var famKey in results.Status) {
      runTaskB = this.getRunButton(this.data.Project, results.Algorithm, results.TestSet, results.MType, famKey, "");
      eCont += `<tr><td style="width:120px; text-align:left;"><b>${famKey}</b></td><td colspan=2>&nbsp;</td><td>${runTaskB}</td></tr>`;
      var compL = results.Status[famKey];
      
      for (var comp=0; comp < compL.length; comp++) {
        if (compL[comp].RS < 0 || compL[comp].RS > 3) compL[comp].RS = 0;
        resIcon = stsIcon[compL[comp].RS]; 
        resIcon = `<img src="/static/images/${resIcon}24.png" width=12/>`;

        var runTaskB = compL[comp].CID ? this.getRunButton(this.data.Project, results.Algorithm, results.TestSet, results.MType, famKey, compL[comp].CID) : "";

        fileProg = (compL[comp].RS == 0) ? "" : `<span class="rFile" onclick="editFile(this);" project="${this.data.Project}"fName="results/${famKey}.${compL[comp].Cmp}/${results.Algorithm}-${results.TestSet}.${results.MType}">${compL[comp].FS}</span>`;
        fileName = compL[comp].Fmy && compL[comp].Cmp ? `${compL[comp].Fmy}.${compL[comp].Cmp}` : "";
        eCont += `<tr><td class="resTD" width=120px;></td><td class="resTD" width=150px;>${resIcon} ${fileName} ${fileProg}</td><td class="resTD" width=350px;>&nbsp;${compL[comp].TS}</td><td>${runTaskB}</td></tr>`;
      }
    }
    eCont +="</table>";
  ...
  postProcess() {
    ...
    var resTimer = setInterval(function() {
      if (!document.hidden && isElementXPercentInViewport(resultElement, 1))
        askASServer(`getResultUpdate {"ID":${self.answerID}}`, self.blockID, function(dID, ans) {
...
function stopCMD(blockID) {
  ...
  if (commandId != null) {
    askASServer("command stop " + commandId, blockID, function(dID, ans){      
...
function processCommand(blockID, vpr) {
  ...
  if (cmd.startsWith("$")) { // ukazi list, status in stop se začnejo z $
    ...
    askASServer("command " + cmd.substring(1), blockID, showServerAnswer);    
  } else { // ostali ukazi (Execute, Analyse, ...)
    askASServer("command run " + cmd, blockID, waitForServerAnswer);  
  }
}
...
function addTask(element) {
  ...
  var com = $(element).attr("computer");
  var mt  = $(element).attr("mt");

  var newTask = `{"Project":"${prj}", "Algorithm":"${alg}", "Testset":"${tst}", "MType":"${mt}", "Family":"${fam}", "ComputerUID":"${com}", "Priority":5}`;
  askASServer("addTask " + newTask.toString(), 0, function (blockID, ans) {
    console.log(ans);
  });
